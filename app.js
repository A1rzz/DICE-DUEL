...
$("create").onclick=async()=>{
  ...
  for(let i=0;i<8;i++){
    ...
    const r=await db.from("games").insert({
      ...
      turn:1,status:"waiting",history:[],winner:null,loser:null,max_players:selectedMode,players,stake_amount:0
    }).select().single();
...
$("saveStake").onclick=async()=>{
  ...
  if(r.error||!r.data)return toast("Betrag konnte nicht gespeichert werden.");
  game=r.data;
  toast("Betrag gespeichert.");
  render();
};

$("shufflePlayers").onclick=async()=>{
  if(my!==1||!game||game.status!=="waiting")return;
  let ps=playersOf(game);
  if(ps.length<2)return toast("Mindestens 2 Spieler werden zum Mischen benötigt.");

  // Fisher-Yates: nur die Reihenfolge wird verändert.
  // Die slot-ID jedes Spielers bleibt gleich, damit alle Geräte synchron bleiben.
  const shuffled=ps.slice();
  for(let i=shuffled.length-1;i>0;i--){
    const j=Math.floor(Math.random()*(i+1));
    [shuffled[i],shuffled[j]]=[shuffled[j],shuffled[i]];
  }
  shuffled.forEach((p,i)=>p.order=i+1);
  ...
  if(r.error||!r.data)return toast("Reihenfolge konnte nicht gemischt werden.");
  game=r.data;
...
function renderLobby(){
  ...
  $("shufflePlayers").classList.toggle("hidden",my!==1||(game.max_players||2)!==10||ps.length<2);
