...
function playersOf(g){
  if(Array.isArray(g.players)&&g.players.length)return g.players;
  const p=[{slot:1,name:g.host_name,active:true}];
  if(g.guest_name)p.push({slot:2,name:g.guest_name,active:true});
  return p;
}
function activePlayers(g){return playersOf(g).filter(p=>p.active!==false)}
function nextActive(g,slot){
  const ps=playersOf(g),max=ps.length;
  for(let i=1;i<=max;i++){
    const next=((slot-1+i)%max)+1;
    const p=ps.find(x=>x.slot===next);
    if(p&&p.active!==false)return next;
  }
  return slot;
}
...
$("create").onclick=async()=>{
  ...
  for(let i=0;i<8;i++){
    const c=code();
    const players=[{slot:1,name,active:true}];
...
$("startGame").onclick=async()=>{
  ...
  const r=await db.from("games").update({
    status:"playing",turn:1,current_number:game.start_number,
    updated_at:new Date().toISOString()
...
function renderPlayers(){
  ...
  playersOf(game).forEach(p=>{
    ...
    d.innerHTML=`<small>P${p.slot}</small><strong></strong><span></span>`;
...
$("roll").onclick=async()=>{
  ...
  if(r.error||!r.data)return toast("Wurf konnte nicht gespeichert werden.");
