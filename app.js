...
function syncExactUI(){
  const currentMode=game?(game.max_players||2):selectedMode;
  document.querySelectorAll(".mode").forEach(x=>{
    x.classList.toggle("active",Number(x.dataset.mode)===currentMode);
  });

  if($("lobbyModeLabel")) $("lobbyModeLabel").textContent=currentMode===10?"Bis zu 10 Spieler":"1 VS 1";

  if(!game){
    if($("sideRoomCode")) $("sideRoomCode").textContent="—";
    if($("sideMode")) $("sideMode").textContent=currentMode===10?"Bis 10":"1 vs 1";
    if($("sideStake")) $("sideStake").textContent="0 €";
    if($("sideStatus")) $("sideStatus").textContent="Start";
    if($("sidePlayers")) $("sidePlayers").textContent="—";
    return;
  }

  const ps=playersOf(game);
  const me=ps.find(p=>p.slot===my);
  if($("profileName")) $("profileName").textContent=(me?.name||game.host_name||"Spieler");
  if($("sideRoomCode")) $("sideRoomCode").textContent=game.room_code||"—";
  if($("sideMode")) $("sideMode").textContent=currentMode===10?"Bis 10":"1 vs 1";
  if($("sideStake")) $("sideStake").textContent=`${game.stake_amount??0} €`;
  if($("sideStatus")) $("sideStatus").textContent=game.status==="waiting"?"Lobby":game.status==="playing"?"Spiel":"Beendet";
  if($("sidePlayers")) $("sidePlayers").textContent=`${ps.length} / ${currentMode}`;
  if($("playerCountTitle")) $("playerCountTitle").textContent=`SPIELER (${ps.length}/${currentMode})`;
}
...
document.querySelectorAll(".mode").forEach(btn=>{
  btn.onclick=()=>{
    ...
    document.querySelectorAll(".mode").forEach(x=>{
      x.classList.toggle("active",Number(x.dataset.mode)===selectedMode);
    });
    syncExactUI();
  };
});
...
if($("stakeAmount")){
  $("stakeAmount").addEventListener("input",()=>{
    if($("sideStake")) $("sideStake").textContent=`${$("stakeAmount").value||0} €`;
  });
}
syncExactUI();
